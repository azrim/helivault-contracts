...
async function main() {
  ...
  console.log(
    chalk.yellow("        Verification Input Extractor Script        "),
  );
  console.log(
    chalk.yellow(
      "============================================================\n",
  ...
  try {
    ...
    if (contractNames.length === 0) {
      throw new Error(
        "No compiled contracts found. Please run 'npx hardhat compile' first.",
      );
    }
  } catch (error) {
    console.error(
      chalk.red.bold("\n❌ Error finding contracts:"),
      error.message,
    );
    process.exit(1);
  }
  ...
  if (
    isNaN(selectedIndex) ||
    selectedIndex < 0 ||
    selectedIndex >= contractNames.length
  ) {
    console.error(
      chalk.red.bold(
  ...
  const contractAddress = await new Promise((resolve) => {
    readline.question(
      chalk.blue.bold(
        `Enter the deployed address for ${chalk.yellow(contractName)}: `,
      ),
      (addr) => {
        readline.close();
        resolve(addr);
  ...
  try {
    ...
    console.log(
      chalk.cyan(`   Output directory cleaned and created at: ${outputDir}`),
    );
  } catch (error) {
    console.error(
      chalk.red.bold("\n❌  Error managing output directory:"),
      error.message,
    );
    process.exit(1);
  }
  ...
  try {
    ...
    if (!latestBuildFile) {
      throw new Error(
        "No build-info file found. Please compile your contract first with 'npx hardhat compile'",
      );
    }

    console.log(
      chalk.cyan(
        `   Found latest build file: ${path.basename(latestBuildFile)}`,
      ),
    );
    ...
    const outputJsonPath = path.join(
      outputDir,
      `${contractName}-verify-input.json`,
    );
    fs.writeFileSync(
      outputJsonPath,
    ...
    console.log(
      chalk.cyan("   File saved to: ") + chalk.white.bold(outputJsonPath),
    );
...
`;
    ...
    const outputMdPath = path.join(
      outputDir,
      `${contractName}-verify-tutorial.md`,
    );
    ...
    console.log(
      chalk.cyan("   File saved to: ") + chalk.white.bold(outputMdPath),
    );
    console.log(
    ...
    console.error(
      chalk.red.bold("\n❌  Error creating verification input:"),
      error.message,
    );
    process.exitCode = 1;
  }
